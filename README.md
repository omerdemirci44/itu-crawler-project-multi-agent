...
This repository contains a CLI-first Python crawler and search system built for a university assignment. It crawls a localhost-accessible website from an origin URL, stores crawl state durably in SQLite, indexes extracted HTML text, and supports search and status inspection through a small command-line interface.
...
- CLI-first MVP with `crawl`, `search`, and `status` commands
...
The MVP is intentionally simple and modular.
...
6. Search and status commands read from the same database.
...
- The interface is CLI-only. There is no HTTP server or browser UI yet.
...
- The current CLI does not implement resume, stop, or server commands.
...
That means a crawl can run in one process or terminal, while `search` or `status` is run against the same database from another process. Newly committed pages become searchable as soon as the crawler finishes writing them.
...
- add a lightweight localhost web UI on top of the same storage layer
...
- the operator interface is available through the CLI
